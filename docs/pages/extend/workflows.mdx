...
`succeeded`, `noop`, `queued`, `waiting`, `blocked`, `degraded`, `dry_run`, or
`failed`. The run API persists that result.
...
- `centaur_workflow_schedule_last_success_timestamp_seconds`, which supports
  per-schedule freshness alerts.
